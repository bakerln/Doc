		...
		public void map(LongWritable key, Text values, Context context)
			...
			if (flag.equals("tmp1")) {
				...
				for (int i = 1; i < tokens.length; i++) {
					...
					Text v = new Text(String.valueOf("A:" + (row) + ","
							+ tokens[i]));
					context.write(k, v);
				}
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			for (Text line : values) {
				...
				if (vals.startsWith("A:")) {
					String[] tokenA = PageRankJob.DELIMITER.split(vals
							.substring(2));

					mapA.put(Integer.parseInt(tokenA[0]),
							Float.parseFloat(tokenA[1]));
				}

				if (vals.startsWith("B:")) {
					String[] tokenB = PageRankJob.DELIMITER.split(vals
							.substring(2));
					mapB.put(Integer.parseInt(tokenB[0]),
							Float.parseFloat(tokenB[1]));
				}
			}
