...
public class HdfsDAO {
	...
	public void transformatFile(String oldFile, String newFile)
			throws IOException {
		// BufferedReader br = new BufferedReader(new FileReader(oldFile));
		FileSystem fs = FileSystem.get(URI.create(hdfsPath), conf);

		FSDataInputStream os = fs.open(new Path(oldFile));
	...
	public void CopyDirectory(String local, String remote) throws IOException {
		FileSystem fs = FileSystem.get(URI.create(hdfsPath), conf);
		// 获取源文件夹当前下的文件或目录
		File[] file = (new File(local)).listFiles();
		for (int i = 0; i < file.length; i++) {
			System.out.println("copy from: " + local +file[i].getName()+ " to " + remote);
			fs.copyFromLocalFile(new Path(local + file[i].getName()), new Path(
	...
	public void upload(String local, String remote) throws IOException {
		Path path = new Path(remote);
		FileSystem fs = FileSystem.get(URI.create(hdfsPath), conf);
		fs.copyFromLocalFile(new Path(local), path);
